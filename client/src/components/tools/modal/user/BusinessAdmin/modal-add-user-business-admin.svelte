<script>
    ...
    //store
    import {userBusinessAllData} from '../../../../../store/user/user.js'
    ...
    const Create = async () =>{
        ...
        let new_user = await CreateUser($loginData._id,email,first_name,last_name,$branch_officeData._id,password,phone,'BUSINESS_ADMIN',address,neighborhood,corner,tuition,vehicle)

        if(new_user.create){
            $userBusinessAllData.push(new_user.data)
