<script>
    ...
    const Create = async () =>{
        ...
        let new_user = await CreateUser($loginData._id,email,first_name,last_name,$branch_officeData._id,password,phone,'CUSTOMER',address,neighborhood,corner,tuition,vehicle)
        ...
        if(new_user.create){
            ...
            let errors = Object.entries(new_user.err.errors)
