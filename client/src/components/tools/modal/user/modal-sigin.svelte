<script>
    ...
    const Create = async () =>{
        ...
        let new_user = await CreateUser('',email,first_name,last_name,BRANCH_ID,password,phone,'CUSTOMER',address,neighborhood,corner,tuition,vehicle)

        console.log('new_user',new_user)

        if(new_user.create){

            $loginData = new_user.data
            $loginActive = new_user.create
            CloseFather()
        }
        else{
            alert(`${new_user.msg}`)
