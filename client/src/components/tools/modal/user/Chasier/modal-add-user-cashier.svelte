<script>
    ...
    //store
    import {userCashierAllData} from '../../../../../store/user/user.js'
    ...
    const Create = async () =>{
        ...
        let new_user = await CreateUser($loginData._id,email,first_name,last_name,$branch_officeData._id,password,phone,'CASHIER',address,neighborhood,corner,tuition,vehicle)

        if(new_user.create){
            $userCashierAllData.push(new_user.data)
            $userCashierAllData = $userCashierAllData
        }
        else{
            alert(new_user.msg)
        }
