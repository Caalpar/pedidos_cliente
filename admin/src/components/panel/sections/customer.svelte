...
<script>
    ...
    // components
    import ModalAddUser from "../../tools/modal/user/Customer/modal-add-user-customer.svelte";
    import ModalDeleteUser from "../../tools/modal/user/Customer/modal-delete-user-customer.svelte";
    import ModalUpdateUser from "../../tools/modal/user/Customer/modal-update-user-customer.svelte";
    import ModalInfoUser from "../../tools/modal/user/Customer/modal-info-user-customer.svelte";
    import ModalEmailUser from "../../tools/modal/user/Customer/modal-email-user-customer.svelte";
    ...
    import {currentIdUser,currentDatauser,userCustomerAllData} from '../../../store/user/user.js'
    ...
    onMount(async ()=>{
        if($branch_officeData){
            let result = await GetAllUsers("CUSTOMER",$branch_officeData._id)
            if(result.success)
                $userCustomerAllData = result.data 
            else
                alert(result.msg)
        }
    })
    ...
    $:{
        branch_office_admin_Array = $userCustomerAllData
    }
    ...
    // Mostrar modal para enviar mail a los usuarios
    let view_email = false

    const OpenModalEmailUser = () =>{
        view_email = true
    }

    const CloseModalEmailUser = () =>
    {
        view_email = false
    }
    ...
    const ChangeBranchOffice = async (branch_office) =>{
        ...
        let result = await GetAllUsers("CUSTOMER",branch_office._id)
...
{/if}
{#if view_email}
    <ModalEmailUser CloseFather = {CloseModalEmailUser} />
{/if}
...
<div class="business-container">
    ...
    <div class="btn-contanier">
        ...
        <div>
            <button class="mail" on:click={()=>{OpenModalEmailUser()}}>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="cube pointer">
