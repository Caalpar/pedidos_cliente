<script>
    ...
    import {currentIdUser,userCashierAllData} from '../../../../../store/user/user.js'
    
    const Delete = async () =>{

        let delete_user = await DeleteUser($currentIdUser,$loginData._id,$loginData.branch_office_id)

        let indexuser = $userCashierAllData.findIndex(b => b._id == $currentIdUser)

        if(indexuser !==-1){
            $userCashierAllData.splice(indexuser,1)
