<script>
    ...
    const Create = async () =>{
        ...
        let new_user = await CreateUser($loginData._id,email,first_name,last_name,$branch_officeData._id,password,phone,'BRANCH_OFFICE_ADMIN',address,neighborhood,corner,tuition,vehicle)

        if(new_user.create){
            $userBranchOfficeAllData.push(new_user.data)
            $userBranchOfficeAllData = $userBranchOfficeAllData
        }
        else{
            alert(`${new_user.msg}`)
        }
