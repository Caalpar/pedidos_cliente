<script>
    ...
    import {currentIdUser,userDeliveryAllData} from '../../../../../store/user/user.js'
    
    const Delete = async () =>{

        let delete_user = await DeleteUser($currentIdUser,$loginData._id,$loginData.branch_office_id)

        let indexuser = $userDeliveryAllData.findIndex(b => b._id == $currentIdUser)

        if(indexuser !==-1){
            $userDeliveryAllData.splice(indexuser,1)
