<script>
    ...
    import {OrderData,OrderDataEdit,OrdersAllData} from '../../../../store/order/oreder.js'
    ...
    onMount(async ()=>{
        ...
        console.log('$OrderDataEdit',$OrderDataEdit)

        id_user = $OrderDataEdit.id_user._id
        user_name = $OrderDataEdit.id_user.first_name +' '+$OrderDataEdit.id_user.last_name

        $ProductsAllData = sort_data
        newOrders = [...$OrderDataEdit.order]

        for (let index = 0; index < newOrders.length; index++) {
            newOrders[index].id = new Date().getTime() + index
        }
    ...
    const Update = async () =>{
        ...
        {
            alert('el pedido tiene que tener como minimo un producto')
            return
        }
...
<ModalContaner title = "Actualizar Pedido" {CloseFather}>
    ...
    <div class="contrainer">
        ...
        <button class="btn-crate" on:click={()=>{Update()}}>Actualizar pedido</button>
        {#if !viewDesktop}
